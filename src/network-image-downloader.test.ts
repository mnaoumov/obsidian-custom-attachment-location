...
function createContext(): TestContext {
  ...
  const createBinary = vi.fn<Vault['createBinary']>().mockImplementation((path) => Promise.resolve(strictProxy<TFile>({ path })));
  const modify = vi.fn<Vault['modify']>().mockResolvedValue(undefined);
  const generateMarkdownLink = vi.fn<FileManager['generateMarkdownLink']>().mockReturnValue('[](assets/image.png)');
  ...
  const app = strictProxy<App>({
    fileManager: strictProxy<FileManager>({ generateMarkdownLink }),
    vault
  });
  ...
  const noteFile = strictProxy<TFile>({ path: 'Research Notes/note.md' });
...
describe('NetworkImageDownloader', () => {
  describe('downloadNetworkImagesForNote', () => {
    ...
    it('should embed the Markdown link generated by Obsidian', async () => {
      ...
      context.getDownloadedImagePath.mockResolvedValue('Research Notes/assets/My Image.png');
      context.generateMarkdownLink.mockReturnValue('[My Image](assets/My%20Image.png)');
      ...
      expect(context.getDownloadedImagePath).toHaveBeenCalledWith(expect.objectContaining({
        fileExtension: 'png',
        fileName: 'My Image',
        noteFilePath: 'Research Notes/note.md'
      }));
      expect(context.createBinary).toHaveBeenCalledWith('Research Notes/assets/My Image.png', expect.any(ArrayBuffer));
      expect(context.generateMarkdownLink).toHaveBeenCalledWith(
        expect.objectContaining({ path: 'Research Notes/assets/My Image.png' }),
        'Research Notes/note.md',
        undefined,
        'My Image'
      );
      expect(context.modify).toHaveBeenCalledWith(context.noteFile, '![My Image](assets/My%20Image.png)');
    });

    it('should embed a wikilink generated by Obsidian', async () => {
      context.cachedRead.mockResolvedValue('![Diagram](https://example.com/diagram.png)');
      mockRequestUrl.mockResolvedValue(createResponse({
        arrayBuffer: toArrayBuffer([0x89, 0x50, 0x4E, 0x47]),
        headers: { 'content-type': 'image/png' }
      }));
      context.generateMarkdownLink.mockReturnValue('[[assets/diagram.png|Diagram]]');

      await context.downloader.downloadNetworkImagesForNote(context.noteFile);

      expect(context.modify).toHaveBeenCalledWith(context.noteFile, '![[assets/diagram.png|Diagram]]');
    });

    it('should not add another embed marker when the generated link is already embedded', async () => {
      context.cachedRead.mockResolvedValue('![Diagram](https://example.com/diagram.png)');
      mockRequestUrl.mockResolvedValue(createResponse({
        arrayBuffer: toArrayBuffer([0x89, 0x50, 0x4E, 0x47]),
        headers: { 'content-type': 'image/png' }
      }));
      context.generateMarkdownLink.mockReturnValue('![Diagram](assets/diagram.png)');

      await context.downloader.downloadNetworkImagesForNote(context.noteFile);

      expect(context.modify).toHaveBeenCalledWith(context.noteFile, '![Diagram](assets/diagram.png)');
    });

    it('should replace only network image matches when the same URL is used elsewhere', async () => {
      const url = 'https://example.com/shared.png';
      context.cachedRead.mockResolvedValue(`![First](${url})\n[Source](${url})\n![Second](${url})`);
      mockRequestUrl.mockResolvedValue(createResponse({
        arrayBuffer: toArrayBuffer([0x89, 0x50, 0x4E, 0x47]),
        headers: { 'content-type': 'image/png' }
      }));
      context.getDownloadedImagePath
        .mockResolvedValueOnce('Research Notes/assets/first.png')
        .mockResolvedValueOnce('Research Notes/assets/second.png');
      ...
      expect(context.generateMarkdownLink).toHaveBeenCalledTimes(2);
    ...
    it('should derive the base name from the URL when the alt text is empty', async () => {
      ...
      expect(context.generateMarkdownLink).toHaveBeenCalledWith(expect.anything(), context.noteFile.path, undefined, undefined);
    });
