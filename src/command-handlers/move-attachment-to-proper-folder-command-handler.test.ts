...
describe('MoveAttachmentToProperFolderCommandHandler', () => {
  ...
  describe('canExecuteAbstractFiles', () => {
    it('should return false when super.canExecute returns false', () => {
      ...
      expect(asPrivate(handler).canExecuteAbstractFiles([createFile('a.png')])).toBe(false);
    });
    ...
    it('should return true when no file is a note', () => {
      ...
      expect(asPrivate(handler).canExecuteAbstractFiles([createFile('a.png'), createFile('b.png')])).toBe(true);
    });

    it('should return true when none of the abstract files are files', () => {
      mockIsFile.mockReturnValue(false);
      expect(asPrivate(handler).canExecuteAbstractFiles([createFolder('folder1'), createFolder('folder2')])).toBe(true);
  ...
  describe('handleMode', () => {
    async function runWithMode(mode: MoveAttachmentToProperFolderUsedByMultipleNotesMode): Promise<void> {
      const attachment = createFile('attachment.png');
      mockIsFile.mockReturnValue(true);
      mockIsFolder.mockReturnValue(false);
      mockIsNoteEx.mockReturnValue(false);
