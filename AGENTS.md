...
- **No suite may disable or remove it.** It outlives each file, and taking it away closes this plugin's gate for every later file in the run. A suite that needs its settings changed proposes them through its `migrateSettings` API and hands the previous values back — see `src/attachment-unit-folder-rescue-cross-plugin.desktop.integration.test.ts`. The store-screenshot captures are the one exception: each owns its vault outright, runs alone in its project and needs renames on from the first frame, so it writes the handler's `data.json` itself, from the seed's exported `ADVANCED_RENAME_AND_DELETE_HANDLER_SEEDED_SETTINGS`.
...
The obsidian-dev-utils base `loadFromFile` writes the full default record to `data.json` when none exists (since 107.0.0; this plugin used to override it to do so). Without that write, nothing on disk tells a fresh install apart from a user who never saved a setting, and a default change moves both.
