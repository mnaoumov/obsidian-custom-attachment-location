...
- **Put the predicate on `canExecuteAbstractFile` and let the base compose it.** `collect-attachments-in-file-command-handler.ts` and `delete-unused-attachments-in-file-command-handler.ts` are the worked examples. `move-attachment-to-proper-folder-command-handler.ts` still has the old shape.
